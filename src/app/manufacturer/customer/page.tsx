...
import { getManufactureCustomers, createManufactureCustomer, getStates } from "@/lib/api";
...
export default function CustomersPage() {
  ...
  async function fetchCustomers(searchTerm = "") {
    try {
      ...
      const data = await getManufactureCustomers(searchTerm || undefined);
          ...
          <div className="divide-y divide-slate-100">
            {customers.map((customer) => (
              <div key={customer.id} className="flex flex-col gap-4 p-5 transition-colors hover:bg-slate-50 sm:flex-row sm:items-center sm:justify-between">
                ...
                {/* Action */}
                <Link 
                  href={`/manufacturer/customer/${customer.id}`} 
