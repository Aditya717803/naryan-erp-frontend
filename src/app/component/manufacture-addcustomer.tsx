...
interface ManufactureAddCustomerProps {
  open: boolean;
  onClose: () => void;
  onSuccess?: () => void | Promise<void>;
}

export default function ManufactureAddCustomer({
  open,
  onClose,
  onSuccess,
}: ManufactureAddCustomerProps) {
  ...
  async function handleCreateCustomer(
    ...
    try {
      ...
      await createManufactureCustomer({
        name: form.name.trim(),
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to create manufacture customer"
      );
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 px-8 py-6">
          <div>
            <h2 className="text-xl font-semibold text-[#0f172a]">
              Add Manufacture Customer
            </h2>

            <p className="mt-1 text-sm text-slate-500">
              Enter the required information to create a new manufacture customer.
            </p>
          </div>
